"""The SQLite store's in-process vector cache must serve repeat queries
without reloading rows, yet pick up a re-index (chunk swap) on the very next
query."""
...
async def test_cache_hits_and_reindex_invalidation(monkeypatch):
    ...
    store.replace_chunks(knowledge.id, _chunks(record.id, ["旧版本内容 v1"]))
    ...
    load_calls = 0
    real_fetch = store._fetch_rows

    def counting_fetch(knowledge_id):
        nonlocal load_calls
        load_calls += 1
        return real_fetch(knowledge_id)

    monkeypatch.setattr(store, "_fetch_rows", counting_fetch)
    ...
    store.replace_chunks(knowledge.id, _chunks(record.id, ["新版本内容 v2"]))
    ...
    store.replace_chunks(knowledge.id, [])
